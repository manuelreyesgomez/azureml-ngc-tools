...
@click.command()
@click.option("--login", is_flag=False, required=True, help="Path to the login config file")
...
def start(login, app):
    ...
    workspace_vm_sizes = [
        (e["name"].lower(), e["gpus"]) for e in workspace_vm_sizes
    ...
    Priority: {vm_priority}
        ...
        if ct_name not in ws.compute_targets:
            ...
            config = AmlCompute.provisioning_configuration(
                ...
                , idle_seconds_before_scaledown=login_config["aml_compute"]["idle_seconds_before_scaledown"]
        ...
        raise Exception("{vm_size} does not have Pascal or above GPU Family".format(vm_size=vm_name))
